...
// gcc -lm  fb_util.c simple_linalg.c fb_sim_pick_place.c -o 6.exe
  ...
  void (*compensation_force)(double *force, const double *load_xy, const double *load_velocity))
{
  ...
  const double sdf_threshold = 0.05;
...
void virtual_force_0(double *force, const double *load_xy, const double *load_velocity, const double *target_xy)
{
  ...
  const double kd_tangential = 100.0;
...
void compensation_force_gravity(double *force, const double *load_xy, const double *load_velocity)
{
  force[0] = 0.0;
  force[1] = 19.6;
}
...
int main(void)
{
  ...
  fb_sim simulation = {.dt=0.001, .fb=fb, .controller=controller_0, .memory=&controller_state, .target_xy={0.1, -1.5},
    .virtual_force=virtual_force_0, .compensation_force=compensation_force_gravity, .torque_last_step={0.0, 0.0}, .torque_time_constant=5e-3};
  
  const unsigned int num_points = 20;
  const double coords[40] = {
    -0.8, -1.1,
    -0.8, -1.0, 0.8, -1.0, 0.8, -1.1,
    0.8, -0.6, -0.8, -0.6, -0.8, -1.1,
    -0.8, -0.2, 0.8, -0.2, 0.8, -1.1,
    0.8, 0.2, -0.8, 0.2, -0.8, -1.1,
    -0.8, 0.6, 0.8, 0.6, 0.8, -1.1,
    0.8, 1.0, -0.8, 1.0, -0.8, -1.1,
    -0.0, -1.0
    };
  const double timeout = 70.0;
  unsigned int current_point = 0;
  while (fb->sim_time < timeout && current_point < num_points) {
    float data[33];
    data[0] = (float)(fb->sim_time);
    for (int j=0; j<28; j++) data[1+j] = (float)(fb->state[j]);
    const double dst_tol = 0.01;
    double dst_err = sqrt((fb->state[6]-coords[2*current_point])*(fb->state[6]-coords[2*current_point]) + (fb->state[7]-coords[2*current_point + 1])*(fb->state[7]-coords[2*current_point + 1]));
    simulation.target_xy[0] = coords[current_point*2];
    simulation.target_xy[1] = coords[current_point*2 + 1];
    ...
    if (dst_err < dst_tol) current_point++;
  }
  printf("end time %f\n", fb->sim_time);

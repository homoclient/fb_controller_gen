...
// gcc -lm  fb_util.c simple_linalg.c fb_sim_random_points.c -o 5.exe
...
int main(void)
{
  ...
  fb_sim simulation = {.dt=0.001, .fb=fb, .controller=controller_0, .memory=&controller_state, .target_xy={0.1, -1.5},
    .virtual_force=virtual_force_0, .compensation_force=NULL, .torque_last_step={0.0, 0.0}, .torque_time_constant=2e-3};
  
  const unsigned int num_points = 3;
  const double coords[6] = {0.01, -1.5, -0.01, -1.5, 0.0, 1.5};
